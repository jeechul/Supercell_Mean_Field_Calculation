...
struct PROFILE
{
    const static int L=432;
    const static int L2 = L*2;
    const static int N_tot = 144;
    constexpr static double RBZ_x=1./6.,RBZ_y=1./6.; // For unit cell : RBZ_x=2, RBZ_y=2
...
// large system //
std::vector<std::vector<int>> PROFILE::lattices = make_lattice(12);
...
int main(int argc, char* argv[])
{
    ...
    const bool IsRev = true;
    EIGEN eigen(L,gamma,IsRev);
    EQUATION equation(eigen,L,15);
    ...
    for (const auto &U : UArr){
        ...
        for(int i = 0; i < Nsam; i++){
            ...
            if(ranks == 0) std::cout << "# duration : " << (std::clock()-start)/CLOCKS_PER_SEC << " s" << std::endl;

            if(flag)
            {
                if(ranks==0) std::cerr<<"# !!!  Roll "<<i<<"-th iteration back !!!"<<std::endl;
                i--; 
                continue;
            }
#ifdef __OUTFILE__		
