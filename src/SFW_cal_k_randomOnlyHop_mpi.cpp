...
struct PROFILE
{
    ...
    constexpr static double sqrt3 = 1.7320508076;
    constexpr static double RBZ_x=1./3.,RBZ_y=1./3./sqrt3; // For unit cell : RBZ_x=2, RBZ_y=2
...
double PROFILE::V = 2.0*sqrt(3.0)*6*6;
...
int main(int argc, char* argv[])
{
    ...
    filename = parser.find<>("path") + "/RandomOnlyHopKagome_SFW_v2-L" + std::to_string(L) + "U" + parser.find<>("U") + "X" + parser.find<>("X") + "N" + std::to_string(N_tot) + ".dat"; 
#ifdef __OUTFILE__
