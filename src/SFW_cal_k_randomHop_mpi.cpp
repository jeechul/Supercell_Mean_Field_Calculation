...
struct PROFILE
{
    ...
    constexpr static double RBZ_x=2./(double)l,RBZ_y=2./(double)l/sqrt(3); // For unit cell : RBZ_x=2, RBZ_y=2
...
int main(int argc, char* argv[])
{
    ...
    {
        filename = parser.find<>("path") + "/RandomHopKagome_SFW_v2-L" + std::to_string(L) + "U" + parser.find<>("U") + "X" + parser.find<>("X") + "N" + std::to_string(N_tot) + ".dat"; 
#ifdef __OUTFILE__
        ...
        do
        {
            ...
            if (ranks==0) std::cout << "# duration : " << (clock()-start)/CLOCKS_PER_SEC << " s" << std::endl;
#ifdef __OUTFILE__		
            if(ranks == 0)
            {
